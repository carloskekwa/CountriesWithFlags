	...
	protected List<Album_feeds> doInBackground(Void... params) {
		...
		try {
			...
			for (int i = 0; i < products.length(); i++) {
				...
				GridViewActivity.mapfeeds.put(_id,new Album_feeds(_id,user,actor,albumid,photoid,type,createdOn_epoch,createdOn,read,edata,modifiedOn));
				album_feed.add(new Album_feeds(_id,user,actor,albumid,photoid,type,createdOn_epoch,createdOn,read,edata,modifiedOn));
				}catch (Exception e){
					System.out.println("Error in the parsing right drawer: ");
					e.printStackTrace();
				}
			}
